...
export function renderCardsView(state) {
    ...
    const disableCardForm = state.accounts.length === 0;
    const lastSynced = state.lastSynced.cards ? `Son güncelleme ${formatDate(state.lastSynced.cards)}` : '';
        ...
        <section class="space-y-6 px-4 py-8 sm:px-6 lg:px-16">
            ...
            <form data-form="create-card" class="grid gap-4 rounded-xl border border-neutral-light-bg/60 bg-white p-6 shadow-sm lg:grid-cols-5 dark:border-neutral-dark-bg dark:bg-neutral-dark-bg/70">
                <div class="lg:col-span-2">
                    ...
                    <select id="cardAccount" name="accountId" required ${disableCardForm ? 'disabled' : ''} class="h-12 w-full rounded-lg border border-neutral-light-bg/60 bg-neutral-light-bg/40 px-3 text-sm text-neutral-text-light focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40 disabled:cursor-not-allowed disabled:opacity-60 dark:border-neutral-dark-bg dark:bg-neutral-dark-bg dark:text-neutral-text-dark">
                        <option value="">${disableCardForm ? 'Önce hesap açmalısınız' : 'Hesap seçiniz'}</option>
                ...
                <label class="lg:col-span-1">
                    ...
                    <input name="cardNumber" required minlength="12" maxlength="19" placeholder="XXXX XXXX XXXX" ${disableCardForm ? 'disabled' : ''} class="h-12 w-full rounded-lg border border-neutral-light-bg/60 bg-neutral-light-bg/40 px-3 text-sm text-neutral-text-light focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40 disabled:cursor-not-allowed disabled:opacity-60 dark:border-neutral-dark-bg dark:bg-neutral-dark-bg dark:text-neutral-text-dark" />
                </label>
                <label>
                    <span class="mb-2 block text-sm font-medium text-neutral-text-light dark:text-neutral-text-dark">CVV</span>
                    <input name="cvv" required minlength="3" maxlength="4" placeholder="123" ${disableCardForm ? 'disabled' : ''} class="h-12 w-full rounded-lg border border-neutral-light-bg/60 bg-neutral-light-bg/40 px-3 text-sm text-neutral-text-light focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40 disabled:cursor-not-allowed disabled:opacity-60 dark:border-neutral-dark-bg dark:bg-neutral-dark-bg dark:text-neutral-text-dark" />
                </label>
                <label>
                    <span class="mb-2 block text-sm font-medium text-neutral-text-light dark:text-neutral-text-dark">Son Kullanma</span>
                    <input name="expiry" required placeholder="AA/YY" pattern="\\d{2}/\\d{2}" ${disableCardForm ? 'disabled' : ''} class="h-12 w-full rounded-lg border border-neutral-light-bg/60 bg-neutral-light-bg/40 px-3 text-sm text-neutral-text-light focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40 disabled:cursor-not-allowed disabled:opacity-60 dark:border-neutral-dark-bg dark:bg-neutral-dark-bg dark:text-neutral-text-dark" />
                </label>
                <div class="lg:col-span-5 flex justify-end">
                    <button type="submit" ${disableCardForm ? 'disabled' : ''} class="flex h-12 items-center justify-center gap-2 rounded-lg bg-primary px-6 text-sm font-semibold text-white shadow transition hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60">
